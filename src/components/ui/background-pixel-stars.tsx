...
// Dark-mode stars are bright pastels that glow on a dark desktop.
...
// Light-mode stars are dark, muted tones so they're visible against the
// pale desktop gradient (bright pastels vanish on a light background).
...
function getTheme(): "light" | "dark" {
  ...
  return (document.body.getAttribute("data-theme") || "light") === "dark"
    ? "dark"
    : "light";
}

function paletteForTheme() {
  return getTheme() === "dark" ? STAR_COLORS_DARK : STAR_COLORS_LIGHT;
}
...
export const BackgroundPixelStars = memo(
  ({ className }: { className?: string }) => {
    ...
    const animateCanvas = useCallback(
      (timestamp: number): void => {
        ...
        if (shootingStarsRef.current.length) {
          ...
          const trailRGB = isDark ? "180, 242, 255" : "70, 100, 150";
          const headColor = isDark ? "#ffffff" : "#27324a";
          shootingStarsRef.current.forEach((star) => {
            ...
            star.trail.forEach((point) => {
              ...
              ctx.fillStyle = `rgba(${trailRGB}, ${point.opacity})`;
    ...
    useEffect(() => {
      ...
      // Recolor stars when the theme toggles (light stars <-> dark stars).
      const themeObserver = new MutationObserver(() => initBackgroundStars());
    ...
    return <canvas ref={canvasRef} className={"pointer-events-none " + (className ?? "fixed inset-0")} />;
  },
  () => true,
);
